...
public:
    ...
    friend double dot(const SparseArray& a, const Array& b);
    
    friend double dot(const Array& a, const SparseArray& b);
...
inline
double dot(const SparseArray& a, const Array& b) {
    double sum = 0.;
    for (unsigned i=0; i<a.numNonZeros(); ++i) {
        int  index = a.readIndex(i);
        double value = a.readValue(i);
        sum += value * b[index];
    }
    return sum;
}
...
inline
double dot(const Array& a, const SparseArray& b) {
    return dot(b, a);
}
