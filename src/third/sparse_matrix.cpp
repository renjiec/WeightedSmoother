...
unsigned SparseMatrix::numLowerNonZeros() const
{
    unsigned N = 0;
    for (unsigned i = 0; i < numRows(); ++i)
    {
        const SparseArray& row = getRow(i);
        for (unsigned j = 0; j < row.numNonZeros(); ++j)
            if (row.readIndex(j) <= (int)i) N++;
    }
    return N;
}
...
unsigned SparseMatrix::copyCRS(long*& rowptr, long*& index, double*& value) const
{
	...
	for (unsigned i=0; i<numRows(); ++i) 
    {
		long int offset = _row[i].numNonZeros();
...
unsigned SparseMatrix::copyCCS(long*& colptr, long*& index, double*& value) const 
{
	...
	for (unsigned i=0; i<numColumns(); ++i) 
    {
		...
		long int offset = col.numNonZeros();
...
SparseMatrix SparseMatrix::multiply(const SparseMatrix& b) const 
{
    ...
    for (unsigned i = 0; i < c.numRows(); ++i) 
    {
        ...
        for (unsigned j = 0; j < c.numColumns(); ++j) 
        {
            ...
            if (std::fabs(value) < 1.0e-10) continue;
            c_i.insert(j, value);
        }
        c.setRow(i, c_i);
    }
